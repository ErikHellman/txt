...
mod tests {
    ...
    #[test]
    fn printable_char() {
        assert_eq!(IH.handle_key(plain(KeyCode::Char('a'))), EditorAction::InsertChar('a'));
        assert_eq!(IH.handle_key(plain(KeyCode::Char('Z'))), EditorAction::InsertChar('Z'));
    }

    #[test]
    fn enter_and_tab() {
        assert_eq!(IH.handle_key(plain(KeyCode::Enter)), EditorAction::InsertNewline);
        assert_eq!(IH.handle_key(plain(KeyCode::Tab)), EditorAction::InsertTab);
    }

    #[test]
    fn backspace_and_delete() {
        assert_eq!(IH.handle_key(plain(KeyCode::Backspace)), EditorAction::DeleteBackward);
        assert_eq!(IH.handle_key(plain(KeyCode::Delete)), EditorAction::DeleteForward);
        assert_eq!(IH.handle_key(ctrl(KeyCode::Backspace)), EditorAction::DeleteWordBackward);
        assert_eq!(IH.handle_key(ctrl(KeyCode::Delete)), EditorAction::DeleteWordForward);
    }

    #[test]
    fn arrow_keys() {
        assert_eq!(IH.handle_key(plain(KeyCode::Up)), EditorAction::MoveCursor(Direction::Up));
        assert_eq!(IH.handle_key(plain(KeyCode::Down)), EditorAction::MoveCursor(Direction::Down));
        assert_eq!(IH.handle_key(plain(KeyCode::Left)), EditorAction::MoveCursor(Direction::Left));
        assert_eq!(IH.handle_key(plain(KeyCode::Right)), EditorAction::MoveCursor(Direction::Right));
    }

    #[test]
    fn shift_arrows_extend_selection() {
        assert_eq!(IH.handle_key(shift(KeyCode::Right)), EditorAction::ExtendSelection(Direction::Right));
        assert_eq!(IH.handle_key(shift(KeyCode::Left)), EditorAction::ExtendSelection(Direction::Left));
    }

    #[test]
    fn ctrl_arrows_word_move() {
        assert_eq!(IH.handle_key(ctrl(KeyCode::Left)), EditorAction::MoveCursorWord(Direction::Left));
        assert_eq!(IH.handle_key(ctrl(KeyCode::Right)), EditorAction::MoveCursorWord(Direction::Right));
    }

    #[test]
    fn ctrl_shift_arrows_word_select() {
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Left)), EditorAction::ExtendSelectionWord(Direction::Left));
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Right)), EditorAction::ExtendSelectionWord(Direction::Right));
    }

    #[test]
    fn home_end() {
        assert_eq!(IH.handle_key(plain(KeyCode::Home)), EditorAction::MoveCursorHome);
        assert_eq!(IH.handle_key(plain(KeyCode::End)), EditorAction::MoveCursorEnd);
        assert_eq!(IH.handle_key(shift(KeyCode::Home)), EditorAction::ExtendSelectionHome);
        assert_eq!(IH.handle_key(ctrl(KeyCode::Home)), EditorAction::MoveCursorFileStart);
        assert_eq!(IH.handle_key(ctrl(KeyCode::End)), EditorAction::MoveCursorFileEnd);
    }
    ...
    fn undo_redo() {
        ...
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Char('Z'))), EditorAction::Redo);
    }
    ...
    fn move_line_alt_arrows() {
        ...
        assert_eq!(IH.handle_key(alt(KeyCode::Down)), EditorAction::MoveLineDown);
    }

    #[test]
    fn page_up_down() {
        assert_eq!(IH.handle_key(plain(KeyCode::PageUp)), EditorAction::MoveCursorPage(Direction::Up));
        assert_eq!(IH.handle_key(plain(KeyCode::PageDown)), EditorAction::MoveCursorPage(Direction::Down));
    }

    #[test]
    fn ast_selection_shortcuts() {
        assert_eq!(IH.handle_key(ctrl(KeyCode::Char('w'))), EditorAction::AstExpandSelection);
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Char('W'))), EditorAction::AstContractSelection);
    }

    #[test]
    fn help_and_search_f_keys() {
        assert_eq!(IH.handle_key(plain(KeyCode::F(1))), EditorAction::ToggleHelp);
        assert_eq!(IH.handle_key(plain(KeyCode::F(3))), EditorAction::SearchNext);
        assert_eq!(IH.handle_key(shift(KeyCode::F(3))), EditorAction::SearchPrev);
    }
    ...
    fn word_wrap_and_search_toggles() {
        ...
        assert_eq!(IH.handle_key(alt_c), EditorAction::SearchToggleCaseSensitive);
    }

    #[test]
    fn command_palette_and_buffer_switcher() {
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Char('P'))), EditorAction::OpenCommandPalette);
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Char('E'))), EditorAction::OpenBufferSwitcher);
    }

    #[test]
    fn toggle_line_comment() {
        assert_eq!(IH.handle_key(ctrl(KeyCode::Char('/'))), EditorAction::ToggleLineComment);
    }

    #[test]
    fn open_recent_files() {
        assert_eq!(IH.handle_key(ctrl(KeyCode::Char('r'))), EditorAction::OpenRecentFiles);
    }

    #[test]
    fn save_file_shortcuts() {
        assert_eq!(IH.handle_key(ctrl(KeyCode::Char('s'))), EditorAction::SaveFile);
        assert_eq!(IH.handle_key(ctrl_shift(KeyCode::Char('S'))), EditorAction::SaveFileAs);
    }
    ...
    fn clipboard_shortcuts() {
        ...
        assert_eq!(IH.handle_key(ctrl(KeyCode::Char('v'))), EditorAction::Paste(String::new()));
    }
    ...
    fn mouse_click_and_drag() {
        ...
        assert_eq!(IH.handle_mouse(click), EditorAction::MouseClick { col: 10, row: 5 });
        ...
        assert_eq!(IH.handle_mouse(drag), EditorAction::MouseDrag { col: 15, row: 6 });
    }
